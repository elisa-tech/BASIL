...
API_PORT=${API_PORT//[^0-9-]/}

### --- main -------------------------------------------------------------------
#  printf 'DGB: $BASIL_API: <%q>\n' "$BASIL_API"
#  printf 'DGB: $SERVER_NAME: <%q>\n' "$SERVER_NAME"
...
cd ~
echo $(pwd)
echo "---  Build Backend API: call clone_basil_repo_if_needed() -------------------"
clone_basil_repo_if_needed "${BASIL_REPOSITORY}" "${BASIL_BUILD_DIR}" ${CLONEBASIL} ${BASIL_GITGOBACK} "${BASIL_COMMITBEFORE}"


## --- COPY local replica of repository to $BASIL_API and change to that directory ------------
echo "---  Build Backend API: delete: ${BASIL_API} if it exist.  ---"
if ! [ -d $BASIL_API ]; then
    mkdir -p $BASIL_API
fi

echo "---  Build Backend API: COPY local copy of repository to: ${BASIL_API} --------"
rcpdir "$BASIL_BUILD_DIR" "$BASIL_API"
cd $BASIL_API

## --- Compare Commit and Reference Date, to compare convert date to seconds since Unix epoch
## --- VERSIONID is not yet in use.
...
# (printf removed; using heredoc below)
...
add_port_to_listen "${APACHE_PORTCONFIG}" ${API_PORT}
...
echo "---  Build Backend API:  Reloading systemd daemon...  -----------------------------------"
...
echo "--- rights of folder: ${BASIL_API}  -------------------------------------------"$'\n'
ls -ld $BASIL_API/
echo "---  $WSGI_SCRIPT  -----------------------------------------------------------------------"$'\n'
cat $WSGI_SCRIPT
