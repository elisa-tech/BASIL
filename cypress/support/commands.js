...
Cypress.Commands.add('check_work_item', (_index,  _type, _obj) => {
  ...
  //Type Check
  card = cy.get(const_data.mapping.table_matching_id)
      .find('tbody').find('tr').eq(0).find('td').eq(1).find('[class=pf-v5-c-card]');
  ...
  card.each(($el, index, $list) => {
    if (index == _index){
      if ((_type == 'test-case') || (_type == 'sw-requirement')){
        cy.wrap($el).find('p[class="work-item-detail-text"]').should('have.text', _obj.description);
      } else if (_type == 'test-specification'){
        cy.wrap($el).find('p[class="work-item-detail-text"]').should('have.text', _obj.test_description);
      }
    }
  })
...
Cypress.Commands.add('delete_work_item', (_index, _type) => {
  ...
  card.each(($el, index, $list) => {
    ...
    if (index == _index) {
      console.log("index: " + index);
      cy.wrap($el).find('button[id^="btn-menu-' + _type + '-delete"]').click();
...
Cypress.Commands.add('assign_work_item', (_parent_index, _index, _parent_type, _type, _obj) => {
  ...
  if (_parent_index > -1){
    ...
    card.each(($el, index, $list) => {
      // $el is a wrapped jQuery element
      if (index == _parent_index) {
        cy.wrap($el).find('button[id*="btn-menu-' + _parent_type + '-assign-' + _type + '-"]').click();
      } else {
        // do something else
      }
    })
...
Cypress.Commands.add('assign_existing_work_item', (_parent_index, _index, _parent_type, _type, _obj_index) => {
  ...
  cy.get('#input-' + _type + '-coverage-0').type('{selectAll}{del}').type('50').should('have.value', '50');
...
Cypress.Commands.add('edit_work_item', (_index, _type, _obj) => {
  ...
  card.each(($el, index, $list) => {
    ...
    if (index == _index) {
      console.log("index: " + index);
      cy.wrap($el).find('button[id^="btn-menu-' + _type + '-edit"]').click();
