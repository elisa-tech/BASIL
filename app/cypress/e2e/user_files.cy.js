...
describe('User Files - Nested Folder Support', () => {
  ...
  it('Create a subfolder for move test', () => {
    ...
    cy.get('#btn-user-file-modal-confirm').click()
    cy.wait(const_data.long_wait)
    cy.get('#table-user-files').find('tbody').contains('move_dest_' + UNIQUE).should('exist')
  })
  ...
  it('Rename a folder', () => {
    ...
    cy.wait(const_data.fast_wait)
    cy.get('#input-rename-name').clear().type('renamed_' + UNIQUE)
    cy.get('#btn-user-file-modal-confirm').click()
    cy.wait(const_data.long_wait)
    cy.get('#table-user-files').find('tbody').contains('renamed_' + UNIQUE).should('exist')
    cy.get('#table-user-files').find('tbody').contains('move_dest_' + UNIQUE).should('not.exist')
  })
  ...
  it('Delete a folder', () => {
    ...
    cy.get('#btn-user-file-modal-confirm').click()
    cy.wait(const_data.long_wait)
    cy.get('#table-user-files').find('tbody').contains('renamed_' + UNIQUE).should('not.exist')
  })
  ...
  it('Delete test folder', () => {
    ...
    cy.get('#btn-user-file-modal-confirm').click()
    cy.wait(const_data.long_wait)
    cy.get('#table-user-files').find('tbody').contains('test_folder_' + UNIQUE).should('not.exist')
  })
})
