...
BASIL allows admins to define custom actions for different work item types.

The actions defined by an admin are available for all users.

The actions are defined in the following way in the admin :doc:`settings`:
...
- ``blank: true`` or undefined means that the action will be opened in a new tab
