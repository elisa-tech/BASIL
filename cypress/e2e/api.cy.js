...
describe('SW Components Dashboard testing', () => {
  ...
  it('Check Table is updated', () => {
    cy.get(const_data.api.table_listing_id).find('tbody').find('tr').should('have.length', 2);
    cy.get(const_data.api.table_listing_id).find('tbody').find('tr').eq(0).find('td').eq(2).invoke('text').then(elem => {
  ...
  it('Create a new version of an existing Sw Component', () => {
    let id;
    cy.get(const_data.api.table_listing_id).find('tbody').find('tr').should('have.length', 2);
    cy.get(const_data.api.table_listing_id).find('tbody').find('tr').eq(0).find('td').eq(6).find('button').click({force: true});
    cy.wait(1000);
    cy.get(const_data.api.table_listing_id).find('tbody').find('tr').eq(0).find('td').eq(1).invoke('text').then(elem => {
        id = elem;
        ...
        cy.get('#input-api-fork-library-version-' + id)
          .type(api_data.first_api_mod_new_version).should('have.value', api_data.first_api_mod_new_version);
    });
    cy.get('#btn-modal-api-confirm').click();
    cy.get(const_data.api.table_listing_id).find('tbody').find('tr').should('have.length', 4);
  })

  it('Search field should filter the SW Components', () => {
    cy.get(const_data.api.table_listing_id).find('tbody').find('tr').should('have.length', 4);
    cy.get('#input-api-list-search').type(api_data.first_mod['library-version'] + ' {del}');
    cy.wait(2000);
    cy.get(const_data.api.table_listing_id).find('tbody').find('tr').should('have.length', 2);
